        ...
        async (msg) => {
          if (msg !== null) {
            ...
            try {
              ...
              await sendOffline(messageContent);
...
async function sendOffline(periodAlarm) {
  try {
    const response = await axios.post(
      "http://localhost:5000/api/Notifcation/offline", // Adjust the base URL/port
      periodAlarm,
      {
        headers: {
...
function adjustAndFormatDate(lastSeen, timeZoneOffsetMinutes) {
  ...
  const adjustedDate = new Date(
    lastSeenDate.getTime() + timeZoneOffsetMinutes * 60000
  );
