...
async function checkDeviceStatus() {
  try {
    ...
    const accountInstruments = await pool.request().query(accountInstrumentQuery);
    ...
    for (const instrument of accountInstruments.recordset) {
      const { AccountInstrumentsID, MAC, IntervalMins, Enable,Message,AccountID,FriendlyName, TimeZone } = instrument;
       ...
       // console.log(latestData);
        ...
        if (latestData.recordset.length > 0) {
          ...
          await pool.request()
            ...
            .input('AccountInstrumentsID', sql.UniqueIdentifier, AccountInstrumentsID)
            .query(`
        ...
        if (isOnline) {
            ...
            const activeAlarms = await pool.request()
              .input('AccountInstrumentsID', sql.UniqueIdentifier, AccountInstrumentsID)
              .query(`
            ...
            if (activeAlarms.recordset[0].Count === 0) {
              ...
              .input('PeriodAlarmID', sql.UniqueIdentifier, periodAlarmID)
                ...
                if(Enable)
                {
                    ...
                    let payload = {periodAlarmID: periodAlarmID, message: Message , accountID : AccountID, lastSeen : lastSeenDate , friendlyName: FriendlyName, timeZone : TimeZone}

                 rabbitChannel.sendToQueue(rabbitQueue, Buffer.from( JSON.stringify(payload)));
                 console.log(`Message published to RabbitMQ queue "${rabbitQueue}":`, payload);
                }
            }
          }
